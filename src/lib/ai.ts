...
export async function summarizeArticle(title: string, content: string): Promise<string | null> {
  ...
  try {
    const { text } = await generateText({
      ...
      prompt: `# 角色
你是一位技术内容编辑，任务是将长篇技术文章压缩为结构化摘要，让读者在 2 分钟内掌握原文需要 20 分钟才能读完的内容，同时不丢失关键技术深度。

# 输出框架

## 1. 概览
用一段话（5-8 句）覆盖以下内容，自然串联，不要分条：
- 这篇文章讲了什么，结论/要点是什么（直接陈述结论本身，不要写"文章讨论了…"）
- 最值得关注的技术点是什么，核心机制或设计选择是什么
- 适合什么背景的人读，需要提前懂什么（写具体知识点）
- 摘要之外是否仍值得读原文，值得的话是哪部分

## 2. 内容梳理
这是输出的主体，占全文 70% 以上篇幅。按文章的自然逻辑顺序，将全文提炼为要点序列。

格式要求——每个要点：

### 要点标题（一句话概括这个段落/章节在说什么）

展开内容，3-6 句话。

内容要求：
- **保留 why**：不要只提炼"作者做了什么"，必须保留"为什么这么做/为什么重要"。缺了 why 的要点是废要点。
- **方案写具体**：如果某个要点涉及技术方案、架构选择、算法思路，写清楚方案本身的机制和关键设计决策，而不是"作者提出了一种方案来解决这个问题"。
- **保留关键数据**：出现在原文中的重要数字（性能、规模、成本、对比结果）直接嵌入对应要点中，不要丢掉。
- **体现思路流向**：要点之间要让读者感受到递进、因果、转折关系。如有必要，在要点开头用一句话衔接上文（"基于上述问题，作者转向……"）。
- **粒度自适应**：文章核心部分拆细（一个关键论述拆成独立要点），铺垫和背景部分压缩合并。总量控制在 6-12 个要点，不设死数。

---

# 原则
1. **信息密度优先**：每句话都要携带实质内容，砍掉一切"这篇文章很有深度"式的废话
2. **忠于原文**：准确传递作者的意思，不添加你的评价和延伸
3. **技术精确**：不因通俗化而引入歧义
